...
class InternetArchiveSource(BaseSource):
    """
    Internet Archive Image Source

    Features:
    - Uses advanced search API
    - Filters open licenses
    - Extracts images from items
    """
        ...
        for query in queries:
            ...
            try:
                ...
                if response.status_code != 200:
                    logging.warning(f"Archive API error: {response.status_code}")
                    continue
                ...
                for doc in docs:
                    ...
                    image_data = self._parse_doc(doc)
                    if image_data:
                        yield image_data
                        collected += 1

            except Exception as e:
                logging.error(f"Archive fetch error: {e}")
                continue
    ...
    def _parse_doc(self, doc: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            # 🔥 Construct image URL (common pattern)
            image_url = f"https://archive.org/download/{identifier}/{identifier}.jpg"

            # 🔥 License filtering
            if not self._is_allowed_license(license_url):
                return None

            return self.build_metadata(
                url=image_url,
                source=self.name,
                license_type=license_url or "open",
                alt_text=title,
                tags=[],
                title=title
            )
            ...
            logging.warning(f"Archive parse error: {e}")
            return None
    ...
    def _is_allowed_license(self, license_url: str) -> bool:
        if not license_url:
            return False

        allowed = ["creativecommons", "publicdomain"]

        return any(x in license_url.lower() for x in allowed)
