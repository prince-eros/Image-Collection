...
class EuropeanaSource(BaseSource):
    """
    Europeana API Source
    - Cultural heritage dataset
    - License filtering applied
    """

    name = "europeana"
    required_api_key = "EUROPEANA_API_KEY"
    source_type = "api" 
    BASE_URL = "https://api.europeana.eu/record/v2/search.json"

    def __init__(self):
        self.api_key = Config.get_api_key("europeana")
        ...
        for query in queries:
            ...
            try:
                ...
                if response.status_code != 200:
                    logging.warning(f"Europeana API error: {response.status_code}")
                    continue
                ...
                logging.error(f"Europeana fetch error: {e}")
                continue
    ...
    def _parse_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            license_info = item.get("rights", [""])[0]
            ...
            logging.warning(f"Europeana parse error: {e}")
            return None
    ...
    def _is_allowed_license(self, license: str) -> bool:
        if not license:
            return False

        allowed = ["creativecommons", "publicdomain", "cc0"]

        return any(x in license.lower() for x in allowed)
