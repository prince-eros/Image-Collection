...
class SmithsonianSource(BaseSource):
    """
    ...
    name = "smithsonian"
    required_api_key = "SMITHSONIAN_API_KEY"
    source_type = "api" 
    BASE_URL = "https://api.si.edu/openaccess/api/v1.0/search"

    def __init__(self):
        self.api_key = Config.get_api_key("smithsonian")
        ...
        for query in queries:
            ...
            try:
                ...
                if response.status_code != 200:
                    logging.warning(f"Smithsonian API error: {response.status_code}")
                    continue
                ...
                logging.error(f"Smithsonian fetch error: {e}")
                continue
    ...
    def _parse_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            media = descriptive.get("online_media", {}).get("media", [])
            ...
            logging.warning(f"Smithsonian parse error: {e}")
            return None
