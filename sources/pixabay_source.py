...
class PixabaySource(BaseSource):
    """
    Pixabay API source with:
    - multi-query support
    - pagination
    - tag extraction (better metadata)
    """

    name = "pixabay"
    required_api_key = "PIXABAY_API_KEY"
    source_type = "api" 
    ...
    def fetch_images(self, bucket: str, limit: int) -> Generator[Dict[str, Any], None, None]:
        ...
        while collected < limit:
            ...
            params = {
                ...
                "page": page,
                "safesearch": "true",
                "min_width": 512  # 🔥 ensures better quality
            }
            ...
            try:
                ...
                if response.status_code != 200:
                    logging.warning(f"Pixabay API error: {response.status_code}")
                    break
                ...
                if not hits:
                    logging.info("No more results from Pixabay")
                    break
    ...
    def _parse_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            logging.warning(f"Pixabay parse error: {e}")
            return None
